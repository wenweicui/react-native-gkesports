...
export class ContactScreen extends WrapScreen {
  ...
  render () {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#fff'}}>
        <View style={{paddingTop:10, borderBottomWidth: 0,borderBottomColor:'rgba(192,192,192,0.3)',...ifIphoneX({marginTop: 50}, {marginTop: 20})}}>
           <View style={{paddingHorizontal:15,height:50,justifyContent:'space-between',flexDirection:'row'}}>
             ...
             <View style={{flex:2.5,alignSelf:'center'}}>
               <Text style={{color:'#5E5959', fontSize:20,fontWeight:'bold',alignSelf:'center'}}>通讯录</Text>
             </View>
             <TouchableOpacity style={{flex:1,alignItems:"flex-end",justifyContent:'center'}} onPress={() => navigation.navigate('newPatientList')}>
               <Icon name="ios-person-add-outline" size={34} color="#5e5959" />
             </TouchableOpacity>
           </View>
        </View>
        <View style={{ borderWidth: 1, borderColor: '#eee', marginHorizontal: 15,marginBottom:10, borderRadius: 50, paddingLeft: isFocused || searchText ? 10 : 0, justifyContent: 'center' }}>
          <TextInput value={searchText} onChangeText={(text) => this.setState({ searchText: text })} style={{ padding: 7, fontSize: 20 , paddingLeft:45}} placeholder={ !searchText ? '搜索' : ''} />
            <View style={{ position: 'absolute', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center',left: 15}}>
              <Icon name='ios-search-outline' size={30} color={'#ccc'} />
            </View>
        </View>
        <ScrollView
          ref='scroll'
