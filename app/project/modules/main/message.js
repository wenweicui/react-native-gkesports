...
let typeList = [
  { nickname: 'Z', picture: '', letter: 'Z' },
  { nickname: 'D', picture: '', letter: 'D' },
  { nickname: 'L', picture: '', letter: 'L' },
  { nickname: 'A', picture: '', letter: 'A' },
  { nickname: 'B', picture: '', letter: 'B' },
  { nickname: 'C', picture: '', letter: 'C' },
]
  ...
  render () {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#fff'}}>
        <View style={[styles.head,{...ifIphoneX({marginTop: 50}, {marginTop: 20})}]}>
          <Text style={{color:'#5e5959', fontSize:30,fontWeight:'bold',alignSelf:'flex-end'}}>消息</Text>
          <TouchableOpacity style={{flex:1,alignItems:"flex-end",justifyContent:'flex-start'}}>
            <Icon name="ios-add-outline" size={36} color="#5e5959" />
          </TouchableOpacity>
        </View>
        <ScrollView
          ref='scroll'
            ...
            <View style={{ }} ref='friendArea'>
              {
                undefined !== typeList && typeList.length > 0 && typeList.map((item, index) => {
                  ...
                  return (checked &&
                    <TouchableOpacity onPress={() => {this.props.navigation.navigate('Chat')}} key={index} ref='friendItem'>
